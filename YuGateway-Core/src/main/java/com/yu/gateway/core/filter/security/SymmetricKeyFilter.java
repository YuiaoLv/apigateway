...
public class SymmetricKeyFilter implements Filter {
	...
	public void doFilter(GatewayContext ctx) throws Exception {
		...
		if (encryptSymmetricKey != null) {
			try {
				...
				if (!jedis.addScoreSet(securityKey, symmetricKey,
						(int) System.currentTimeMillis() + SYMMETRIC_KEY_EXPIRE_TIME)) {
	...
	public String getUserId(GatewayContext ctx) {
		...
		String userId = (String) Objects.requireNonNull(JWTUtil.getClaimByToken(token, TOKEN_SECRET))
				.get(TOKEN_USERID_KEY);
